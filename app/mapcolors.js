...
module.exports = (function() {
    ...
    return function (colorConfig, m) {
        ...
        var colorOptions = ["highColor", "lowColor", "zeroColor", "hoverColor","noDataColor"];
        ...
        c.applyStateColors = function (duration, callback) {
            ...
            var externalLabelStates = function (arr) {
                ...
                return o;
            } (["NH", "VT", "MA", "RI", "CT", "NJ", "DE", "MD", "DC", "HI", "US","GU","PR","VI"]);
            ...
            for (state in c.stateColors) {
                if (c.stateColors.hasOwnProperty(state)) {
                    if (m.stateObjs[state]) {
                        ...
                        m.stateObjs[state].attr("stroke", m.colorConfig.borderColor);
                        m.stateObjs[state].attr("stroke-width", m.borderWidth);
                        ...
                        if (m.stateLabelObjs[state]) {
                            ...
                            if (brightness(c.stateColors[state]) >= m.brightnessThreshold || externalLabelStates[state]) {
                                m.stateLabelObjs[state].attr("fill", m.labelDarkColor);
                            } else {
                                m.stateLabelObjs[state].attr("fill", m.labelLightColor);
                            }
            ...
            for (state in c.stateColors) {
                if (c.stateColors.hasOwnProperty(state)) {
                    if (m.stateObjs[state]) {
                        ...
                        var thisNode = m.stateObjs[state].node();
                        var thisLabel = m.stateLabelObjs[state].node();
                        if (m.highlightBorder === true && m.data[state][m.highlightIndex] === 1) {
                            thisNode.parentNode.appendChild(thisNode);
                        }
                        thisLabel.parentNode.appendChild(thisLabel);
                    }
                }
            }
